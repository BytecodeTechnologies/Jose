<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ShowBlog.aspx.cs" Inherits="StableLawFirm.ShowBlog" %>
						...
						<div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 pull-left blog-page-main-content">
							<div class="single-news-postTwo">
								...
								<div class="post">
									...
									<a href="#"><h3><%# DataBinder.Eval(Container.DataItem, "Title")%></h3></a>
									...
									<p style="white-space: pre-wrap;"><%# DataBinder.Eval(Container.DataItem, "Text")%></p>
									...
									<div class="comment-area">
										<h6>Comment &nbsp; (<asp:Label ID="TotalComment" runat="server" Text="Label"></asp:Label>)</h6>

					
                                        <asp:Repeater runat="server" ID="comments"><ItemTemplate>
										<div class="single-comment clear-fix">
											<div class="img-container pull-left">
												
												<p><%# DataBinder.Eval(Container.DataItem, "Comment_By")%> <i class="fa fa-long-arrow-right" aria-hidden="true"></i></p>
											</div>
											<div class="text pull-left">
												<span><%# DataBinder.Eval(Container.DataItem, "Comment_Date_St")%></span>
												<p style="white-space: pre-wrap;"><%# DataBinder.Eval(Container.DataItem, "Comment_Text")%></p>
											</div>
										</div> <!-- /.single-comment -->
                                            </ItemTemplate></asp:Repeater>
									</div> <!-- /.comment-area -->
										...
										<form action="#">
											<div class="row">
												<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                                                    <p>
													<input class="required" type="text" id="txtcommentName" placeholder="Name">
                                                    <span id="lblvalidName"></span>
                                                        </p>
												</div>
												<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                                                    <p>
													<input type="email" class="required" id="txtCommentEmail" placeholder="Email">
                                                     <span id="lblvalidEmail"></span>
                                                        </p>
												</div>
												<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                                    <p>
													<textarea placeholder="Comment" class="required" id="txtCommentText"></textarea>
                                                       <span id="lblValidComment"></span>
                                                        </p>
												</div>
											</div>

											<%--<button class="tran3s p-color-bg hvr-bounce-to-rightTwo">POST COMMENT</button>--%>
                                                   <a id="btnPostComment" style="color:white" class="theme-button p-color-bg hvr-bounce-to-rightTwo">Post Comment</a>

										</form>
									</div>
								</div> <!-- /.post -->
							</div> <!-- /.single-news-postTwo -->
						</div> <!-- /.blog-page-main-content -->
						<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 side-bar side-bar-style-two pull-right">
							<div class="wrapper-fix">

								<%--<form action="#" class="sidebar-search">
									<input type="text" id="SearchText" placeholder="Search">
									<button id="SearchBlog"><i class="fa fa-search-minus" aria-hidden="true"></i></button>
								</form>--%>
								...
								<div class="blog-categories">
									...
									<ul>
                                           <asp:Repeater ID="Repeater1" runat="server">
                                               <ItemTemplate>
										<li><a onclick="SearchByID(<%# DataBinder.Eval(Container.DataItem, "categoryId")%>)"  class="tran3s"><i class="fa fa-long-arrow-right" aria-hidden="true"></i><%# DataBinder.Eval(Container.DataItem, "CategoryName")%></a></li>
...
<script>
    ...
    $("#btnPostComment").click(function (evt) {
        $("#btnPostComment").parent().next(".validation").remove();
        $('#btnSaveResult').parent().next(".validation").remove();
        var focusSet = false;
        if (!$("#txtcommentName").val().trim()) {
            if ($("#lblvalidName").parent().next(".validation").length == 0) // only add if not added
                   {
                $("#lblvalidName").parent().after("<div class='validation' style='color:red;margin-bottom: 20px;'>Please Result Name</div>");
                   }
            $("#txtcommentName").focus();
                   focusSet = true;
               }
               else {
            $("#lblvalidName").parent().next(".validation").remove(); // remove it
               }
        if (!$("#txtCommentEmail").val().trim()) {
            if ($("#lblvalidEmail").parent().next(".validation").length == 0) // only add if not added
                {
                $("#lblvalidEmail").parent().after("<div class='validation' style='color:red;margin-bottom: 20px;'>Please enter Email</div>");
            }
         

               }
               else {
            $("#lblvalidEmail").parent().next(".validation").remove(); // remove it
        }

        if (!$("#txtCommentText").val().trim()) {
            if ($("#lblValidComment").parent().next(".validation").length == 0) // only add if not added
            {
                $("#lblValidComment").parent().after("<div class='validation' style='color:red;margin-bottom: 20px;'>Please enter Comment</div>");
            }


        }
        else {
            $("#lblValidComment").parent().next(".validation").remove(); // remove it
        }

        var fullname = $("#txtcommentName").val();
        var Email = $("#txtCommentEmail").val();
        var Text = $("#txtCommentText").val();
        var blogid = $('#<%= hdnBlogId.ClientID %>').val();
        if (fullname == "" || Email == "" || Text =="" || blogid =="") {
            return;
            $("#btnPostComment").focus()
        }
        ...
        $.ajax({
            type: "POST",
            contentType: "application/json; charset=utf-8",
            url: "/Showblog.aspx/ADDComment",
            data: JSON.stringify({ fullname: fullname, Email: Email, Text: Text, blogid: blogid }),
            dataType: "Json",
            success: function (response) {
               
                $("#btnPostComment").parent().after("<div id='showmessage' class='validation' style='color:red;margin-bottom: 20px;'>Your comment has been successfully update. Comment will be shown once approved from Admin.</div>");
                $("#txtcommentName").val('');
                $("#txtCommentEmail").val('');
                $("#txtCommentText").val('');
            },
    ...
    function SearchByID(id) {
        window.location.href = "Blog.aspx?CategoryId=" + id;
    };
